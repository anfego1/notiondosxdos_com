...
/* Creating a blank array in which we'll store an object for each deal fetched from the CRM */
const dealsArray = []

/* Creating a function for making requests to the Zoho API.
...
async function getDeal() {
  ...
  They correspond to actual DEAL numbers */
  const start = 1
  const end = 330

  /* This 'for' loop will make the first set of requests to the ZohoCRM.
  
  We're using a basic 'for (let i = num)' loop because it will correspond to specific deal numbers. */
...
.replace(' ', '_')}_(Pokémon)`
      ...
      /* Push our pokeData object onto the end of the dealsArray array. This is done each time our loop executes, resulting in an array full of objects - one for each Pokemon that you included in the loop (using the start and end numbers).
  ...
  We now have all of the Pokemon we'll sent to Notion in dealsArray, so we'll now use a for...of loop to loop through that array,  get the 'species' info for each element from PokeAPI, and add each piece of info to that pokemon's object in dealsArray. */
  for (let pokemon of dealsArray) {

    /* Just like we did above, here we use axios.get() to call the PokeAPI endpoint we want. Note that this time we're passing the pokemon.number property from the current element of dealsArray (which is stored in the pokemon variable created in this loop) into the PokeAPI URL. */
  ...
  /* Once both loops have finished running, we call the createNotionPage() function which is defined below. It's important to note that we're calling this function within the getDeal() function. 
  
  Since getDeal() is an async function, calling createNotionPage() outside of it (in the global context) will cause createNotionPage() to run before getDeal() can finish construcing its array of objects.
...
/* Here's where we actually call the getDeal() function. When you type `node index.js` in the Terminal to run this script, it immediately runs this function, which kicks off everything else.
...
/* Create a function for sending our data to the Notion API. As with getDeal(), this function has to be async because it is using axios.get(), which is an asynchronous method that returns a Promise first. Therefore, we must await it, and to do that it has to be inside an async function. */
async function createNotionPage() {

  /* Here's our main loop for the process of sending data to Notion. We already have our array of pokemon objects (dealsArray), so we can use a for...of loop to iterate through it. 
...
const zohoApiKey = process.env.ZOHO_API;
const zohoApiUrl = "https://crm.zoho.eu/"; 
//idk if it's correct ApiUrl: https://www.zoho.com/es-xl/crm/help/api/using-api-url.html

async function updateRecordInZohoCrm(recordId, moduleName, fieldName, fieldValue) {
  const headers = {
  ...
  try {
    const response = await fetch(`${zohoApiUrl}/${moduleName}/${recordId}`, {
      method: "PUT",
      headers,
...
const notionPageDateProperty = "Date Property"; // Replace with your Notion page date property name
const zohoRecordId = "your-zoho-record-id";
const zohoModuleName = "Leads"; // Replace with your Zoho CRM module name
const zohoFieldName = "Date_Field"; // Replace with your Zoho CRM field name

notion.pages.retrieve({
  page_id: notionPageId
}).then((response) => {
  const notionPage = response.result;
  const datePropertyValue = notionPage.properties[notionPageDateProperty].date.start;

  updateRecordInZohoCrm(zohoRecordId, zohoModuleName, zohoFieldName, datePropertyValue);
}).catch((error) => {
